...
class Trainer(object):
    def __init__(self, epoch, index_list, args):
        ...
        if args.use_balanced_weights:
            ...
            print('check weightsssss:',weight)
        else:
            weight = None
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            # todo 先用target给搞成合适的维度再说，batch size * 4 * 512 * 512,这个时候已经是个torch了
            # todo 不用这么做，不合理
            ...
            if self.args.cuda:
                image, target,batch_mask = image.cuda(), target.cuda(),mask.cuda()
            self.scheduler(self.optimizer, i, epoch, self.best_pred)
            self.optimizer.zero_grad()
            output = self.model(image)
            # print('check output size :',output.shape)  #torch.Size([batch size, 4, 512, 512])

            # 用mask去掉背景部分,在计算损失函数的时候再去掉背景的区域
            # output *= batch_mask
            # print('mask * output shape ==',output.shape)
            # exit()
            # loss_ce,loss_dice = self.criterion(output, target)  # 一个batch的loss，然后做反向传播
            # 这里变成return ce loss和class dice
            loss_ce,class_dice = self.criterion(output,target,mask)
            ...
            loss = loss_ce  + x * loss_dice  # 逐步加上dice loss的权重
            ...
            # pred += 1
            # print('check pred before : ', np.sum(pred == 0), np.sum(pred == 1))
            # # print('ppppp:',pred[0,:10,:10])
            # # print('mmmm:',mask[0,:10,:10])
            # # print('predddd',np.unique(pred))  # 这里是4个类别了
            # # Add batch sample into evaluator
            # # print('target shape', target.shape)
            # # print('pred shape', pred.shape)  # 盲猜是 batch size * 512 * 512
            # mask = mask.cpu().numpy()
            # mask = mask.astype(np.int64)
            # pred *= mask
            # print('maskskkkk:',np.sum(mask == 0))
            # print('check pred : ',np.sum(pred == 0),np.sum(pred == 1))
    ...
    def validation(self, epoch):  # 验证集，每10个epoch求平均
        ...
        for i, sample in enumerate(tbar):
            ...
            # todo 把mask弄成维度为：batch size * 4 * 512 * 512,这个时候已经是个torch了
            # for i in range(mask.shape[0]):
            #     aa = torch.stack([mask[i],mask[i],mask[i],mask[i]],dim=0)
            #     aa = torch.unsqueeze(aa,dim= 0 )
            #     print('check aa',aa.shape)
        ...
        self.balance_m_weights = self.cal_weights(mious=class_miou[1:])   #更新balance miou weights,这里只有4个类别了
...
def main():
    ...
    parser.add_argument('--backbone', type=str, default='resnet',       #注意注意
    ...
    parser.add_argument('--batch-size', type=int, default=8,     # todo 注意注意！！
                        metavar='N', help='input batch size for \
                                training (default: auto)')
    parser.add_argument('--test-batch-size', type=int, default=8,
                        metavar='N', help='input batch size for \
                                testing (default: auto)')
    parser.add_argument('--use-balanced-weights', action='store_true', default=True,    # todo true  注意注意
