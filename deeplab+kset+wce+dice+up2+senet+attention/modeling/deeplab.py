...
class DeepLab(nn.Module):
    def __init__(self, backbone='resnet', output_stride=16, num_classes=5,
        ...
        # todo 上采样采用反卷积恢复decoder 的 output feature尺寸   # 这些其实都没有用到，没有用到的东西就不要占一个参数了
        ...
        # self.se = SELayer(256,16)    # 没有用到的层不要弄出来，会有参数在这里的
    ...
    def forward(self, input):
        ...
        # # todo 对 encoder结束后，ASPP之前的特征做se 通道注意力机制
        # x = self.se(x)    # 这里已经是32*32的feature了，小区块的信息基本没了。加在这里秋茄反而更加不能识别，小区块的识别也不好
        # todo 对low_level_feat做se通道注意力机制
        # low_level_feat = self.se(low_level_feat)  # 这里加se的话，会导致图片边缘黑色区域很容易误报的，说明这个维度的特征，携带大量背景信息，如果加强的话，反而不好

        x = self.aspp(x)
        # print('aspp',x.size())                     # torch.Size([1, 256, 32, 32])
        x = self.decoder(x, low_level_feat)   # 128                                   # 这里输出已经是5个类别了，我要怎么把mask的信息加进来？
        ...
        #todo 添加 SEnet模块，在这里添加就好了吧
        # x = self.se
        #  不对，他这里已经是5维了，那应该在变成5维之前加这个模块，5维已经是分类了
