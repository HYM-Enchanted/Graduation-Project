...
def main():
    ...
    parser.add_argument('--in-path', type=str, default='/mnt/mangrove_cut0/cut0-2000', help='image to test')
    parser.add_argument('--out-path', type=str, default='/mnt/mangrove_cut0/2class-2000',
                        help='mask image to save')
    parser.add_argument('--backbone', type=str, default='resnet',
                        choices=['resnet', 'xception', 'drn', 'mobilenet'],
                        help='backbone name (default: resnet)')
    parser.add_argument('--ckpt', type=str,
                        default='/mnt/code/deeplab+kset+wce+dice+up2+senet+2class/run/mangrove/cut0_sea_2class/model_best.pth.tar',
                        help='saved model')
    ...
    for name in os.listdir(args.in_path):
      ...
      #  band2 = image['image'][:, :, 1]
        # band4 = image['image'][:, :, 3]
       # image = np.dstack([band2,band2,band2])
        # image = image['image'][:, :, 1:]
    ...
    end_time = time.time()
    print('predice {} images use time {}'.format(len(os.listdir(args.in_path)),end_time-start_time))
