...
#修改回只有ce loss，从train_wce+x-dice-w-test.py中复制过来的
# todo 改了ce loss和dice loss的反向传播，都会计算，但是只有ce loss参与反向传播。
# todo 把class balance的地方修改为false
...
class Trainer(object):
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            loss = loss_ce    # + x * loss_dice  # 只要ce loss，但是也会算一下dice loss，做对比嘛
    ...
    def validation(self, epoch):  # 验证集，每10个epoch求平均
        ...
        for i, sample in enumerate(tbar):
            ...
            loss = loss_ce   #+ x * loss_dice  # 只要ce loss， dice loss会算，但是不用
            test_loss += loss.item()
